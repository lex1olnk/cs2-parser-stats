...
async function getTopPlayers(tournamentId: string) {
  ...
  const rows: any[] = await prisma.$queryRaw`
    SELECT
      ...
      COALESCE(r.rounds,   1)::int  AS rounds
    FROM (
      SELECT player_id, SUM(kills) AS kills, SUM(hs_kills) AS hs_kills
      FROM kill_stats_per_match
      WHERE tournament_id = ${tournamentId}::uuid
      GROUP BY player_id
    ) k
    JOIN "user" u ON u.id = k.player_id
    LEFT JOIN (
      SELECT player_id, SUM(damage) AS damage
      FROM damage_stats_per_match
      WHERE tournament_id = ${tournamentId}::uuid
      GROUP BY player_id
    ) d ON d.player_id = k.player_id
    LEFT JOIN (
      SELECT player_id, SUM(rounds) AS rounds
      FROM rounds_played_per_match
      WHERE tournament_id = ${tournamentId}::uuid
      GROUP BY player_id
    ) r ON r.player_id = k.player_id
    ...
    ORDER BY k.kills::float / GREATEST(r.rounds, 1) DESC
    LIMIT 15
  `;
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      rating: rounds > 0 ? kills / rounds : 0,
    };
  });
}
          ...
          <aside className="col-span-3 hidden lg:block space-y-12">
            <div>
              ...
              <div className="space-y-4 font-mono text-[10px]">
                <div className="flex gap-4 items-center">
                  ...
                  <span className="text-zinc-400">K/R &gt; 1.20 (ELITE)</span>
                </div>
                <div className="flex gap-4 items-center">
                  <div className="w-2 h-2 bg-zinc-800" />
                  <span className="text-zinc-600">AVERAGE_ZONE (0.80)</span>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 border border-zinc-900 bg-zinc-900/10">
              <p className="text-[9px] leading-relaxed text-zinc-500 font-mono italic">
                Rating = K/R (убийства за раунд). ADR = урон за раунд. HS% =
